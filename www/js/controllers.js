...
.controller('TopCtrl', function($scope,DataMusicone,$timeout,$ionicHistory,$state,$ionicModal,$ionicLoading, $moment,$rootScope,$window) {
...
.directive("player", function($compile) {
    return {
        ...
        link: function(scope, element, attrs) {
            scope.played = attrs.player;

            console.log(attrs.player);
        },
       //  template: '<div class="bar bar-footer bar-calm musicone-top_player" > <div>'+
       //   ' <progress  class="range-calm" ng-value="currentTime / duration || 0"></progress><div><span>'+
       //     '<button id="musicone-top_play" ng-click="previous()" class="button button-icon ion-skip-backward"> </button> '+
       //    '<button id="musicone-top_play" ng-click="playerAction(change);playPause(change);" class="button button-icon" ng-class='{"ion-play ": ( false ==  player) , "ion-stop" : (true == player)}'> </button> '+
       //    '<button id="musicone-top_play"ng-click="next()" class="button button-icon ion-skip-forward"></button>'+ 
       //     ' <span id="musicone-time" ng-bind="currentTime | secondsToDateTime | date:mm:ss"></span>'+
       // ' </span></div></div></div>'
    ...
    compile: function (element, attrs) {
        ...
        elem = element;
          ...
          return function(scope,element){
             var x = angular.element('<div class="bar bar-footer bar-calm musicone-top_player" > <div>'+
       ' <progress  class="range-calm" ng-value="currentTime / duration || 0"></progress><div><span>'+
         '<button id="musicone-top_play" ng-click="previous()" class="button button-icon ion-skip-backward"> </button> '+
         '<button id="musicone-top_play" ng-click="playerAction(change);playPause(change);" class="button button-icon" ><span ng-if="!played" class="ion-play"></span><span ng-if="played" class="ion-pause"></span></button> '+
         '<button id="musicone-top_play"ng-click="next()" class="button button-icon ion-skip-forward"></button>'+ 
           // ' <span id="musicone-time" >{{currentTime | secondsToDateTime | date:mm:ss}}</span>'+
        ' </span></div></div></div>');
                element.append(x);
                $compile(x)(scope);
